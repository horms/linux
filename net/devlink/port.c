...
static int devlink_port_fn_rx_gro_hw_fill(struct devlink_port *devlink_port,
					  struct nla_bitfield32 *caps,
					  struct netlink_ext_ack *extack)
{
	bool is_enable;
	...
	if (!devlink_port->ops->port_fn_rx_gro_hw_get ||
	    devlink_port->attrs.flavour != DEVLINK_PORT_FLAVOUR_PCI_VF)
		return 0;

	err = devlink_port->ops->port_fn_rx_gro_hw_get(devlink_port,
						       &is_enable, extack);
	if (err) {
		if (err == -EOPNOTSUPP)
			return 0;
...
static int
devlink_port_fn_rx_gro_hw_set(struct devlink_port *devlink_port, bool enable,
			      struct netlink_ext_ack *extack)
{
	return devlink_port->ops->port_fn_rx_gro_hw_set(devlink_port, enable,
							extack);
}
				    ...
				    struct netlink_ext_ack *extack)
{
	...
	if (caps.selector & DEVLINK_PORT_FN_CAP_RX_GRO_HW) {
