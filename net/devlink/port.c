...
static int devlink_port_fn_header_split_fill(struct devlink_port *devlink_port,
					     struct nla_bitfield32 *caps,
					     struct netlink_ext_ack *extack)
{
	bool is_enable;
	...
	if (!devlink_port->ops->port_fn_header_split_get ||
	    devlink_port->attrs.flavour != DEVLINK_PORT_FLAVOUR_PCI_VF)
		return 0;

	err = devlink_port->ops->port_fn_header_split_get(devlink_port,
							  &is_enable, extack);
	if (err) {
		if (err == -EOPNOTSUPP)
			return 0;
...
static int devlink_port_fn_gro_fill(struct devlink_port *devlink_port,
				    struct nla_bitfield32 *caps,
				    struct netlink_ext_ack *extack)
{
	bool is_enable;
	...
	if (!devlink_port->ops->port_fn_gro_get ||
	    devlink_port->attrs.flavour != DEVLINK_PORT_FLAVOUR_PCI_VF)
		return 0;

	err = devlink_port->ops->port_fn_gro_get(devlink_port,
						 &is_enable, extack);
	if (err) {
		if (err == -EOPNOTSUPP)
			return 0;
				     ...
				     bool *msg_updated)
{
	...
	err = devlink_port_fn_header_split_fill(devlink_port, &caps, extack);
	if (err)
		return err;

	err = devlink_port_fn_gro_fill(devlink_port, &caps, extack);
	if (err)
		return err;
...
static int
devlink_port_fn_header_split_set(struct devlink_port *devlink_port, bool enable,
				 struct netlink_ext_ack *extack)
{
	return devlink_port->ops->port_fn_header_split_set(devlink_port,
							   enable, extack);
}

static int
devlink_port_fn_gro_set(struct devlink_port *devlink_port, bool enable,
			struct netlink_ext_ack *extack)
{
	return devlink_port->ops->port_fn_gro_set(devlink_port, enable, extack);
}
				    ...
				    struct netlink_ext_ack *extack)
{
	...
	if (caps.selector & DEVLINK_PORT_FN_CAP_HEADER_SPLIT) {
		err = devlink_port_fn_header_split_set(devlink_port,
						       caps_value &
						       DEVLINK_PORT_FN_CAP_HEADER_SPLIT,
						       extack);
		if (err)
			return err;
	}
	if (caps.selector & DEVLINK_PORT_FN_CAP_GRO) {
		err = devlink_port_fn_gro_set(devlink_port, caps_value &
					      DEVLINK_PORT_FN_CAP_GRO, extack);
		if (err)
			return err;
	}
	return 0;
					  ...
					  struct netlink_ext_ack *extack)
{
	...
	if (attr) {
		...
		if (caps.selector & DEVLINK_PORT_FN_CAP_HEADER_SPLIT) {
			if (!ops->port_fn_header_split_set) {
				NL_SET_ERR_MSG_ATTR(extack, attr,
						    "Port doesn't support header_split function attribute");
				return -EOPNOTSUPP;
			}
			if (devlink_port->attrs.flavour != DEVLINK_PORT_FLAVOUR_PCI_VF) {
				NL_SET_ERR_MSG_ATTR(extack, attr,
						    "header_split function attribute supported for VFs only");
				return -EOPNOTSUPP;
			}
		}
		if (caps.selector & DEVLINK_PORT_FN_CAP_GRO) {
			if (!ops->port_fn_gro_set) {
				NL_SET_ERR_MSG_ATTR(extack, attr,
						    "Port doesn't support gro function attribute");
				return -EOPNOTSUPP;
			}
			if (devlink_port->attrs.flavour != DEVLINK_PORT_FLAVOUR_PCI_VF) {
				NL_SET_ERR_MSG_ATTR(extack, attr,
